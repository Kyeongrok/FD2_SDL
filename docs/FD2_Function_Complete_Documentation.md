# FD2.EXE 역공학 함수 완전 설명 문서

본 문서는 IDA Pro 디스어셈블 분석과 decompile 도구로 추출한 의사 코드를 기반으로, FD2.EXE 안의 **1228개** 역공학 함수의 완전한 목록을 담고 있다.
...
주소 순으로 정렬된 완전한 함수 목록:

| 주소 | 함수명 | 호출자 수 | 피호출 수 | 원형 |
|------|--------|----------|----------|------|
...
### 호출자 수순 정렬 (다른 함수를 가장 많이 호출하는 함수)
...
| 주소 범위 | 추정 기능 영역 |
|----------|-----------|
| 0x10000 - 0x107FF | 프로그램 진입과 초기화 |
| 0x10800 - 0x10FFF | 게임 데이터 로드 |
| 0x11000 - 0x11FFF | 입력 처리와 메인 루프 |
| 0x12000 - 0x13FFF | 렌더링과 그리기 |
| 0x14000 - 0x17FFF | 게임 로직과 전투 |
| 0x18000 - 0x1FFFF | 유닛 처리와 애니메이션 |
| 0x20000 - 0x23FFF | 게임 메인 루프 함수군 |
| 0x24000 - 0x2FFFF | 확장 게임 로직 |
| 0x30000 - 0x33FFF | UI와 메뉴 함수 |
| 0x34000 - 0x36FFF | 보조 함수 |
| 0x37000 - 0x3FFFF | 시스템 레벨 함수 (메모리 관리 등) |
| 0x40000 - 0x44FFF | 데이터 테이블과 상수 |
| 0x4A000 - 0x4EFFF | 유틸리티 함수와 보조 |
